...
void Window::Run() {
    ...
    std::cout << "Press F to cast a ray and detect objects." << std::endl;
...
void Window::key_callback_ray_casting() {
    // 按F键进行射线投射检测
    static int lastFState = GLFW_RELEASE;
    int fState = glfwGetKey(this->m_window, GLFW_KEY_F);
    
    if (fState == GLFW_PRESS && lastFState == GLFW_RELEASE) {
        castRayAndDetectObjects();
    }
    lastFState = fState;
}
...
void Window::key_callback_bezier() {
    ...
    if (rightState == GLFW_PRESS && lastRightState == GLFW_RELEASE) {
        ...
        // 规范化屏幕坐标到 NDC
        float x = (2.0f * (float)xpos) / (float)m_width - 1.0f;
        float y = 1.0f - (2.0f * (float)ypos) / (float)m_height;
        ...
        // 增大包围球半径以便射线更容易命中（避免必须贴脸删除）
        ctrlObj->SetBoundingSphereRadius(0.25f);
